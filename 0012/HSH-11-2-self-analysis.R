...
ageg_religion_marriage1_mo <- welfare %>%
  filter(!is.na(group_marriage) & ageg != "young") %>% 
  count(ageg, religion, group_marriage) %>% 
  group_by(ageg, religion) %>%
  mutate(pct = round(n / sum(n) * 100, 1))

ageg_religion_marriage1_mo

df_divorce_mo <- ageg_religion_marriage %>%
  filter(group_marriage == "divorce"& ageg != "young") %>%
  select(ageg, religion, pct) 

df_divorce_mo

ggplot(data = df_divorce_mo, aes(x = ageg, y = pct, fill = religion )) + 
  geom_col(position = "dodge")
  
# 지역별 연령대 분석. 노년층이 많은 지역은?
...
welfare %>%
  select(code_region, region) %>% head
